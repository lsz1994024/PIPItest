...
public class BuildIndex {
    ...
    public BuildIndex(Map<String, String> parameterMap, String labelling, boolean needCoding, boolean addDecoy, boolean addContaminant) throws Exception {
        ...
        String dbPath = parameterMap.get("db");   //fasta database
        ...
        Map<String, String> proteinPeptideMap; // the sequences in the fasta file
        DbTool contaminantsDb = null;
        if (addContaminant) {
            contaminantsDb = new DbTool(null, "contaminants"); // dont need file path, the contaminants is default
        ...
        // build database
        inferSegment = new InferSegment(massTool, parameterMap, fixModMap); // build up the template of 4851 and set some predefined PTM mass shift
        ...
        for (String peptide : peptideMassMap.keySet()) {
            ...
            SparseBooleanVector code2 = null;
            SparseBooleanVector code4 = null;
            if (needCoding) {
                code = inferSegment.generateSegmentBooleanVector(DbTool.getSequenceOnly(peptide));
                code2 = inferSegment.generateSegmentBooleanVector2(DbTool.getSequenceOnly(peptide));
                code4 = inferSegment.generateSegmentBooleanVector4(DbTool.getSequenceOnly(peptide));
            }
            ...
            if (leftRightFlank != null) {
                tempMap.put(peptide, new Peptide0(code, code2, code4, isTarget(peptideProteinMap.get(peptide)), peptideProteinMap.get(peptide).toArray(new String[0]), leftRightFlank[0], leftRightFlank[1]));
        ...
        // peptide0Map is a database that contains all of the peptides and the corresponding code, including real/decoy/contaminants
