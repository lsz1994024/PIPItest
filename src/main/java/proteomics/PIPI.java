...
import proteomics.Types.PercolatorEntry;
...
public class PIPI {
    ...
    public static void main(String[] args) {
        ...
        // Set parameters
        String parameterPath = args[0].trim(); // parameter.def
        String spectraPath = args[1].trim();   //.mgf  .mzXml
//        int singleScanStr = Integer.parseInt(args[2].trim());
        ...
        try {
            ...
            new PIPI(parameterPath, spectraPath, dbName); //SQL db
        } catch (Exception ex) {
            ex.printStackTrace();
    ...
    private PIPI(String parameterPath, String spectraPath, String dbName) throws Exception {
        ...
        // prepare spectra is to convert spectrum to SQL database
        PreSpectra preSpectra = new PreSpectra(spectraParser, ms1Tolerance, ms1ToleranceUnit, massTool, ext, msLevelSet, sqlPath); // it's a sql db for spectra?
...
//            System.out.println(scanNum);
        ...
        PreparedStatement sqlPreparedStatement = sqlConnection.prepareStatement("REPLACE INTO spectraTable (scanNum, scanId, precursorCharge, precursorMass, mgfTitle, isotopeCorrectionNum, ms1PearsonCorrelationCoefficient, labelling, peptide, theoMass, isDecoy, globalRank, normalizedCorrelationCoefficient, score, deltaLCn, deltaCn, matchedPeakNum, ionFrac, matchedHighestIntensityFrac, explainedAaFrac, otherPtmPatterns, aScore, PTMFreeCandidates, PTMOnlyCandidates) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            ...
            for (Future<PIPIWrap.Entry> task : taskList) {
                if (task.isDone()) {
                    if (task.get() != null) {
                        ...
                        sqlPreparedStatement.setString(23, entry.PTMFreeCandidates);
                        sqlPreparedStatement.setString(24, entry.PTMOnlyCandidates);
    ...
    private static Map<Integer, PercolatorEntry> runPercolator(String percolatorPath, String percolatorInputFileName, String percolatorOutputFileName, String percolatorProteinOutputFileName, String tdFastaPath, String enzymeName) throws Exception {
        ...
        if ((new File(percolatorInputFileName)).exists()) {
            ...
            switch (enzymeName) {
                case "Trypsin":
                    percolatorEnzyme = "trypsin";
                    break;
                case "Trypsin/P":
                    percolatorEnzyme = "trypsinp";
                    break;
                case "TrypsinR":
                    percolatorEnzyme = "trypsin";
                    break;
                case "LysC":
                    percolatorEnzyme = "lys-c";
                    break;
                case "ArgC":
                    percolatorEnzyme = "arg-c";
                    break;
                case "Chymotrypsin":
                    percolatorEnzyme = "chymotrypsin";
                    break;
                case "GluC":
                    percolatorEnzyme = "glu-c";
                    break;
                case "LysN":
                    percolatorEnzyme = "lys-n";
                    break;
                case "AspN":
                    percolatorEnzyme = "asp-n";
                    break;
                default:
                    percolatorEnzyme = "trypsin";
                    break;
    ...
    private void writeFinalResult(Map<Integer, PercolatorEntry> percolatorResultMap, String outputPath, Map<String, Peptide0> peptide0Map, String sqlPath) throws IOException, SQLException {
        ...
        if (percolatorResultMap == null) {
            writer.write("scan_num,peptide,charge,theo_mass,exp_mass,abs_ppm,A_score,protein_ID,score,delta_C_n,other_PTM_patterns,MGF_title,labelling,isotope_correction,MS1_pearson_correlation_coefficient, PTM_Free_Candidates, PTM_Only_Candidates\n");
        } else {
            writer.write("scan_num,peptide,charge,theo_mass,exp_mass,abs_ppm,A_score,protein_ID,score,delta_C_n,percolator_score,posterior_error_prob,q_value,other_PTM_patterns,MGF_title,labelling,isotope_correction,MS1_pearson_correlation_coefficient, PTM_Free_Candidates, PTM_Only_Candidates\n");
        }

        Connection sqlConnection = DriverManager.getConnection(sqlPath);
        Statement sqlStatement = sqlConnection.createStatement();
        ResultSet sqlResultSet = sqlStatement.executeQuery("SELECT scanNum, precursorCharge, precursorMass, mgfTitle, isotopeCorrectionNum, ms1PearsonCorrelationCoefficient, labelling, peptide, theoMass, isDecoy, score, otherPtmPatterns, aScore, deltaCn, PTMFreeCandidates, PTMOnlyCandidates FROM spectraTable");
        while (sqlResultSet.next()) {
            int isDecoy = sqlResultSet.getInt("isDecoy");
            if (!sqlResultSet.wasNull()) {
                if (isDecoy == 0) {
                    ...
                    if (percolatorResultMap == null) {
                        double score = sqlResultSet.getDouble("score");
                        String str = String.format(Locale.US, "%d,%s,%d,%f,%f,%f,%s,%s,%f,%f,%s,\"%s\",%s,%d,%f,%s,%s\n", scanNum, peptide, sqlResultSet.getInt("precursorCharge"), theoMass, expMass, ppm, aScore, String.join(";", proteinIdSet).replaceAll(",", "~"), score, sqlResultSet.getDouble("deltaCn"), sqlResultSet.getString("otherPtmPatterns"), sqlResultSet.getString("mgfTitle"), sqlResultSet.getString("labelling"), sqlResultSet.getInt("isotopeCorrectionNum"), sqlResultSet.getDouble("ms1PearsonCorrelationCoefficient"), sqlResultSet.getString("PTMFreeCandidates"), sqlResultSet.getString("PTMOnlyCandidates"));
